...
const BuildingIcon = () => (
  <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{marginRight: '8px', verticalAlign: 'middle', color: '#fbbf24'}}>
...
2. **性格分類**: 必ず「DiSC理論」と「アマサイ（アクティブ・マネージ・サービス・イノベーション）」の**両方**を用いて、相手の性格タイプを分析し、「DiSC：〇〇型、アマサイ：〇〇型」と明記してください。
3. **商談シナリオの構成（超重要）**: シナリオは単なる箇条書きを避け、営業がそのままトークとしてイメージできるレベルで肉付けしてください。**必ず「未来・問題・原因・解決」のサイクルを、連続して2回（1周目：浅い課題・期待 → 2周目：より深く本質的な課題・理想）回す構成**にし、全体で400〜600文字程度のボリュームで詳細に描写してください。
   その際、1周目も2周目も必ず出力内で以下の項目名（カッコ表記）を見出しとして使って明記してください。（例：「▼1周目」「【未来】...」「【問題】...」「【原因】...」「【解決】...」「▼2周目」「【未来】...」...）
   - 【未来】究極の理想。痛みが解決された先に待っている理想と、それを成し遂げた次に手が届く理想（数珠繋ぎでイメージできる最高の世界）。
   - 【問題】理想を叶えるためのボトルネック。放置すると待っている最悪の将来（一度地獄に落とすイメージ）を提示。
   - 【原因】問題はあなたのせいではないと伝え、第三者（国の制度、法律、業界の悪習、既得権益などの共通の敵）を原因として提示。
   - 【解決】その根本原因を解決できる唯一の手段が、今回提案する商品サービスであるという着地。
4. **具体性**: 各項目は現場ですぐ使える鋭い内容にしてください。
...
必ず指定されたJSON形式で、短く鋭い日本語で回答してください。`;
  ...
  const handleCorpProfile = async () => {
    if (!corpUrl.trim() && !corpIndustry.trim() && !corpImage && !corpProduct.trim()) {
      setCorpError('何か情報を入力するか、画像を添付してください。');
      return;
    }

    setIsCorpProfiling(true);
    setCorpResult(null);
    setCorpError('');

    const prompt = `あなたはトップクラスの経営コンサルタント・法人営業ストラテジストです。
提供された企業情報や決算書画像から、経営層（決裁者）に刺さる課題感と攻略シナリオを分析してください。
【絶対遵守ルール】
1. 現場レベルの小さな悩みではなく、組織全体・経営レベルの大きな痛み（コスト、人材不足、成長鈍化など）にフォーカスすること。
2. 専門用語を並べるだけでなく、経営層に直感的に刺さる、営業マンが明日すぐ使える生きたわかりやすい言葉で出力すること。
3. 業界の一般的なトレンドに逃げるのではなく、提供された企業のHPURLや事業内容から直接的に推測できる「その企業特有のダイレクトな痛み」を具体的に言語化すること。
4. シナリオは「未来・問題・原因・解決」のフレームワークに沿って、400〜500文字程度で深く詳細に肉付けすること。
【提案予定の商材・サービス】
${corpProduct.trim() || '（指定なし）'}

【企業のURL・事業内容】
${corpUrl.trim() || '（指定なし）'}

【業界の状況・競合の情報】
${corpIndustry.trim() || '（指定なし）'}

必ず指定されたJSON形式で、詳細な日本語で回答してください。`;

    try {
      const result = await generateGeminiContent(prompt, 'corporate', corpImage, corpProduct);
      setCorpResult(result);
    } catch (error) {
      if (error.message.includes('401') || error.message.includes('合言葉')) {
        localStorage.removeItem('app_password');
        setIsAuthenticated(false);
      }
      setCorpError(error.message);
    } finally {
      setIsCorpProfiling(false);
    }
  };
...
必ず指定されたJSON形式で、短く的確な日本語で回答してください。`;
  ...
  const isCorpProfileDisabled = !corpUrl.trim() && !corpIndustry.trim() && !corpImage && !corpProduct.trim();
  ...
  return (
    <div className="app-container">
      ...
      <div className="tabs-container">
        ...
        <button className={`tab-btn ${activeTab === 'corporate' ? 'active' : ''}`} onClick={() => setActiveTab('corporate')}>
          🏢 企業分析
        </button>
        <button className={`tab-btn ${activeTab === 'followup' ? 'active' : ''}`} onClick={() => setActiveTab('followup')}>
          ✉️ フォローアップ
        </button>
      </div>
      ...
      <main className="modules-grid">
            ...
            {profileResult && (
              <div className="result-area" id="profile-result-area">
                <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem'}}>
                  <h3 style={{margin: 0, color: '#60a5fa'}}>👤 担当者分析結果</h3>
                  <button 
                ...
                <div className="result-section">
                  ...
                  <div className="result-value" style={{fontSize: '0.95rem', lineHeight: '1.6', background: 'rgba(255,255,255,0.05)', padding: '1rem', borderRadius: '8px', borderLeft: '3px solid #60a5fa', whiteSpace: 'pre-wrap'}}>
        ...
        {/* Module 2: Corporate Profiling */}
        {activeTab === 'corporate' && (
          <section className="glass-panel slide-in">
            <h2 className="panel-title" style={{color: '#fbbf24'}}>
              <BuildingIcon /> 企業・法人プロファイリング
            </h2>
            
            <div style={{marginBottom: '1.5rem'}}>
              <div className="input-group" style={{marginBottom: '1rem'}}>
                <label htmlFor="corpProduct" style={{color: '#fcd34d'}}>📦 提案予定の商材・サービス</label>
                <input
                  id="corpProduct"
                  type="text"
                  className="input-field"
                  placeholder="例：法人向けDX支援パッケージ"
                  value={corpProduct}
                  onChange={(e) => setCorpProduct(e.target.value)}
                  style={{borderColor: 'rgba(251, 191, 36, 0.4)'}}
                />
              </div>

              <div className="input-group" style={{marginBottom: '1rem'}}>
                <label htmlFor="corpUrl">🌐 企業のURL・事業内容など</label>
                <input
                  id="corpUrl"
                  type="text"
                  className="input-field"
                  placeholder="コーポレートサイトのURLや、事業の概要..."
                  value={corpUrl}
                  onChange={(e) => setCorpUrl(e.target.value)}
                />
              </div>

              <div className="input-group" style={{marginBottom: '1rem'}}>
                <label htmlFor="corpIndustry">📊 業界の状況・トレンド</label>
                <textarea
                  id="corpIndustry"
                  className="input-field"
                  rows={2}
                  placeholder="例：2024年問題で人材不足が深刻化している"
                  value={corpIndustry}
              ...
              <div className="input-group" style={{marginBottom: '0'}}>
                <label><ImageIcon /> 🖼️ 決算書・IR資料・組織図の添付</label>
                <div style={{display: 'flex', alignItems: 'center', marginTop: '0.5rem'}}>
                  <input 
                    type="file" 
                    accept="image/*" 
                    onChange={(e) => handleImageChange(e, setCorpImage)} 
                    style={{fontSize: '0.85rem', color: '#cbd5e1'}}
                  />
                </div>
              </div>
            ...
            <button 
              className="btn" 
              onClick={handleCorpProfile} 
              disabled={isCorpProfiling || isCorpProfileDisabled}
            ...
            {corpResult && (
              <div className="result-area" id="corp-result-area" style={{ borderColor: 'rgba(251, 191, 36, 0.3)' }}>
                <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem'}}>
                  <h3 style={{margin: 0, color: '#fbbf24'}}>🏢 企業分析結果</h3>
                  <button 
                    onClick={() => handleCapture('corp-result-area', '企業・法人プロファイリング結果')}
                    style={{background: 'rgba(255,255,255,0.1)', border: 'none', color: 'white', padding: '0.4rem 0.8rem', borderRadius: '4px', cursor: 'pointer', fontSize: '0.85rem', display: 'flex', alignItems: 'center'}}
                  >
                    <ImageIcon /> 画像として保存
                  </button>
                ...
                <div className="result-section">
                  <div className="result-label" style={{ color: '#fcd34d' }}>⚠️ 組織全体の最大の課題（痛み）</div>
                  <div className="result-value">
                    <ul style={{ margin: 0, paddingLeft: '1.2rem', fontSize: '0.95rem' }}>
                      {corpResult.corporatePain && Array.isArray(corpResult.corporatePain) ? (
                        corpResult.corporatePain.map((item, i) => (
                          <li key={i} style={{marginBottom: '0.5rem', color: '#e2e8f0'}}>{item}</li>
                        ))
                      ) : (
                        <li style={{color: '#94a3b8'}}>課題の再抽出を行ってください。</li>
                ...
                <div className="result-section">
                  <div className="result-label danger-text">
                    <AlertIcon /> 外部環境リスク（放置すると致命傷になる脅威）
                  </div>
                  <div className="result-value">
                    <ul style={{ margin: 0, paddingLeft: '1.2rem', fontSize: '0.95rem' }}>
                      {corpResult.externalRisks && Array.isArray(corpResult.externalRisks) ? (
                        corpResult.externalRisks.map((item, i) => (
                          <li key={i} className="danger-text" style={{marginBottom: '0.3rem'}}>{item}</li>
                        ))
                      ) : (
                        <li>（情報なし）</li>
                ...
                <div className="result-section">
                  ...
                  <div className="result-value" style={{fontSize: '0.95rem', lineHeight: '1.6', background: 'rgba(255,255,255,0.05)', padding: '1rem', borderRadius: '8px', borderLeft: '3px solid #fbbf24', whiteSpace: 'pre-wrap'}}>
                    {corpResult.scenario || 'シナリオ案を作成中... 再試行してください。'}
                  </div>
                </div>

                <div className="result-section">
                  <div className="result-label" style={{ color: '#fcd34d' }}>💡 経営層・決裁者向けキラーフレーズ</div>
                  <div className="result-value" style={{fontWeight: 'bold', color: '#fcd34d', fontSize: '1.1rem'}}>{corpResult.killerPhrase || 'フレーズ考案中...'}</div>
                </div>
              </div>
            )}
          </section>
          ...
          <section className="glass-panel slide-in">
            ...
            {followUpResult && (
              <div className="result-area" id="followup-result-area" style={{ borderColor: 'rgba(139, 92, 246, 0.3)' }}>
                <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem'}}>
                  <h3 style={{margin: 0, color: '#c4b5fd'}}>✉️ フォローアップ案</h3>
                  <button 
